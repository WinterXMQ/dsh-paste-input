window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // --- 可选的粘贴记录持久化（sessionStorage；默认关） ---
    const PERSIST_KEY = 'dsh-paste-input.records';
    const PERSIST_FLAG_KEY = 'dsh-paste-input.persistRecords';
    const PERSIST_MAX_FILE_BYTES = 1024 * 1024; // 单文件超过 1 MiB 不持久化
    const PERSIST_TOTAL_BYTES = 3 * 1024 * 1024; // 快照总量上限 3 MiB
    let persistRecords = false;
    try { persistRecords = window.localStorage.getItem(PERSIST_FLAG_KEY) === 'on'; } catch (e) { /* 存储不可用 */ }
    function setPersistFlag(next) {
      persistRecords = next;
      try { window.localStorage.setItem(PERSIST_FLAG_KEY, next ? 'on' : 'off'); } catch (e) { /* best effort */ }
    ...
    function persistSnapshot() {
      ...
      try {
        ...
        for (const [ref, record] of records) {
          ...
          if (persist === undefined || persist.dataUrl.length > PERSIST_MAX_FILE_BYTES) continue;
          total += persist.dataUrl.length;
          if (total > PERSIST_TOTAL_BYTES) break;
          out.push([ref, { label: record.label, sessionId: record.sessionId, persist }]);
        }
        window.sessionStorage.setItem(PERSIST_KEY, JSON.stringify(out));
      } catch (e) { /* 配额不足：持久化是尽力而为 */ }
    }
    ...
    function attachPersist(record) {
      const file = record.items !== undefined ? record.items[0]?.file : undefined;
      if (file === undefined || file.size > PERSIST_MAX_FILE_BYTES) return;
      fileToDataUrl(file).then(dataUrl => {
        record.persist = { name: file.name, type: file.type, dataUrl };
        persistSnapshot();
      }).catch(() => { /* best effort */ });
    }
    function restorePersisted() {
      ...
      try {
        ...
        if (raw === null) return;
        for (const [ref, saved] of JSON.parse(raw)) {
          if (records.has(ref)) continue;
          const file = new File([base64ToBytes(String(saved.persist.dataUrl))], saved.persist.name, { type: saved.persist.type });
          records.set(ref, { ref, sessionId: saved.sessionId, items: [{ file, path: saved.label }], total: file.size, label: saved.label, status: 'ready', uploaded: 0, persist: saved.persist });
        }
      } catch (e) { /* 恢复是尽力而为 */ }
    }
    restorePersisted();
    ...
    const css = `
      ...
      /* image attachments: hover thumbnail + fullscreen viewer */
      .dshca-chat-chip[data-image="1"],.dshca-chip[data-image="1"]{cursor:zoom-in}
      .dshca-hover-card{display:none;position:absolute;bottom:calc(100% + 8px);left:0;z-index:60;box-sizing:border-box;padding:6px;border:1px solid var(--dsw-alias-border-l1);border-radius:10px;background:var(--dsw-alias-bg-base);box-shadow:var(--dsw-shadow-lv3);pointer-events:none}
      .dshca-hover-card img{display:block;max-width:280px;max-height:200px;border-radius:6px;object-fit:contain}
      .dshca-hover-name{max-width:280px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-top:4px;color:var(--dsw-alias-label-caption);font-size:11px}
      [data-image="1"]:hover .dshca-hover-card{display:block}
      .dshca-viewer-overlay{position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;background:rgba(0,0,0,.86)}
      .dshca-viewer-bar{display:flex;align-items:center;gap:8px;padding:10px 14px;color:#fff;font-size:13px}
      .dshca-viewer-name{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .dshca-viewer-scale{flex:none;font-variant-numeric:tabular-nums;opacity:.8;min-width:44px;text-align:right}
      .dshca-viewer-hint{flex:none;opacity:.55;font-size:12px}
      .dshca-viewer-btn{flex:none;height:30px;min-width:30px;padding:0 10px;border:1px solid rgba(255,255,255,.25);border-radius:8px;background:rgba(255,255,255,.08);color:#fff;font:inherit;font-size:13px;cursor:pointer}
      .dshca-viewer-btn:hover{background:rgba(255,255,255,.18)}
      .dshca-viewer-body{flex:1;position:relative;overflow:hidden;cursor:grab;touch-action:none}
      .dshca-viewer-body[data-dragging="1"]{cursor:grabbing}
      .dshca-viewer-body img{position:absolute;left:50%;top:50%;max-width:92%;max-height:92%;user-select:none;-webkit-user-drag:none;transform-origin:center center}
    `;
    ...
    /** Composer chip label: the file name alone (official @file references match). */
    function composerLabel(label) {
    ...
    // Pasted files get a unified base name — images paste_image.<ext>, other
    // files paste_file.<ext> — with the (2), (3)… de-duplication handled by
    // add()'s uniqueLabel against the live composer chips. Only the PASTE
    // path renames; drops and the file/folder picker keep their real names.
    const MIME_EXTENSIONS = {
      'image/png': '.png', 'image/jpeg': '.jpg', 'image/gif': '.gif',
      'image/webp': '.webp', 'image/bmp': '.bmp', 'image/svg+xml': '.svg',
      'image/avif': '.avif', 'text/plain': '.txt', 'application/pdf': '.pdf',
    };
    function renamedPastedItems(items) {
      return items.map(item => {
        const isImage = item.file.type.startsWith('image/');
        const original = item.path;
        const dot = original.lastIndexOf('.');
        const extension = dot > 0 ? original.slice(dot) : (MIME_EXTENSIONS[item.file.type] ?? '');
        return { ...item, path: `${isImage ? 'paste_image' : 'paste_file'}${extension}` };
      });
    }
    ...
    // ── image attachments: hover thumbnail + click viewer (zoom & pan) ─────
    const IMAGE_PREVIEW_EXTENSIONS = new Set(['png', 'jpg', 'jpeg', 'gif', 'webp', 'bmp', 'avif', 'ico']);

    function isImagePath(path) {
      const dot = path.lastIndexOf('.');
      return IMAGE_PREVIEW_EXTENSIONS.has(dot < 0 ? '' : path.slice(dot + 1).toLowerCase());
    }

    /** Host-served, marker-validated image URL for an already-committed send file. */
    function attachmentFileUrl(root, path) {
      return `${API}/file?root=${encodeURIComponent(root)}&path=${encodeURIComponent(path)}`;
    }

    /** Blob URLs for composer-side File objects (one per File, page lifetime). */
    const fileObjectUrls = new WeakMap();
    function objectUrlOf(file) {
      let url = fileObjectUrls.get(file);
      if (url === undefined) {
    ...
    /**
     * Fullscreen image viewer: wheel zoom towards the cursor, drag to pan,
     * zoom buttons, double-click toggle 1×/2×, reset, copy path, Esc/backdrop
     * (click without drag) to close. GIFs keep animating — the img element is
     * never re-created, only transformed.
     */
    function openImageViewer(options) {
      const overlay = document.createElement('div');
      overlay.className = 'dshca-viewer-overlay';
      const bar = document.createElement('div');
      bar.className = 'dshca-viewer-bar';
      const name = document.createElement('span');
      name.className = 'dshca-viewer-name';
      name.textContent = options.name ?? '';
      name.title = options.path ?? options.name ?? '';
      const hint = document.createElement('span');
      hint.className = 'dshca-viewer-hint';
      hint.textContent = '滚轮缩放 · 拖动移动 · 双击 1×/2×';
      const scaleLabel = document.createElement('span');
      scaleLabel.className = 'dshca-viewer-scale';
      const makeButton = (text, onClick) => {
        const button = document.createElement('button');
        button.type = 'button';
        button.className = 'dshca-viewer-btn';
        button.textContent = text;
        button.addEventListener('click', onClick);
        return button;
      };
      const copyButton = options.path === undefined ? null : makeButton('复制路径', () => {
        navigator.clipboard?.writeText(options.path).then(
          () => showToast('已复制路径：' + options.path),
          () => showToast('复制失败：' + options.path),
        );
      });
      const body = document.createElement('div');
      body.className = 'dshca-viewer-body';
      const image = document.createElement('img');
      image.alt = options.name ?? 'attachment';
      ...
      const onKey = event => {
        ...
        else if (event.key === '-') zoomAt(window.innerWidth / 2, window.innerHeight / 2, 1 / 1.25);
        else if (event.key === '0') reset();
      };
      window.addEventListener('keydown', onKey);

      // Backdrop click closes — but only a real click, not the tail of a pan.
      body.addEventListener('click', event => {
        if (event.target === body && moved < 4) close();
      });

      if (copyButton !== null) bar.appendChild(copyButton);
      bar.appendChild(hint);
      bar.appendChild(name);
      bar.appendChild(makeButton('−', () => zoomAt(window.innerWidth / 2, window.innerHeight / 2, 1 / 1.25)));
      bar.appendChild(scaleLabel);
      bar.appendChild(makeButton('+', () => zoomAt(window.innerWidth / 2, window.innerHeight / 2, 1.25)));
      bar.appendChild(makeButton('重置', reset));
      bar.appendChild(makeButton('关闭 ×', close));
    ...
    function AttachButton(props) {
      ...
      // conversation.input.left provides no owner props; props.input is always
      // undefined here (the dock slot does get InputZone with input). Default
      // to 'plain' (enabled) — add() itself guards the phase and surfaces errors.
    ...
    function AttachmentChips(props, className) {
      ...
      return h('div', { className }, ...occurrences.map(occurrence => {
        ...
        // Composer-side preview: when the record holds image files, hover
        // shows a thumbnail of the first one (blob URL — bytes are still
        // local) and clicking the chip opens the zoom/pan viewer. Non-image
        // records and the remove button keep their existing behavior.
        const imageItem = status !== 'missing'
          ? (record?.items ?? []).find(item => isImagePath(item.path))
          : undefined;
        return h('div', {
          className: 'dshca-chip',
          'data-status': status,
          'data-image': imageItem === undefined ? undefined : '1',
          key: occurrence.occurrenceId,
          onClick: imageItem === undefined ? undefined : event => {
            if (event.target.closest?.('.dshca-remove') != null) return;
          ...
          imageItem !== undefined && h('div', { className: 'dshca-hover-card', key: 'preview' },
            h('img', { src: objectUrlOf(imageItem.file), alt: imageItem.path }),
            h('div', { className: 'dshca-hover-name' }, `${imageItem.path} · 点击查看大图`)),
          h('button', {
            type: 'button',
            className: 'dshca-remove',
            'aria-label': `Remove ${record?.label ?? occurrence.label}`,
            // NOTE: no `busy` here — it belongs to other components' scope;
            // referencing it crashed the whole dock slot the moment a chip
            // rendered (ReferenceError swallowed by the slot's error boundary).
    ...
    function parseAttachmentBlock(text) {
      ...
      // Both end-marker spellings exist in real history: current bundles write
      // '==== END DSH_PASTE_INPUT ====' while older cached bundles wrote the
      // V1-suffixed '==== END DSH_PASTE_INPUT_V1 ===='; accept both. The V1
      // spelling is legacy-only and may be dropped in a future release. Legacy
      // unmarked history is intentionally not folded. Prefix matching: user
      // words typed right after the attachment chip can land on the same line
      // as the end marker.
      ...
      const endMatch = /^(==== END DSH_PASTE_INPUT(?:_V1)? ====)/.exec(lines[endIdx].trim());
      ...
      for (const block of blocks) {
        ...
        for (const file of block.files) {
          ...
          if (isImagePath(file.path)) {
            // Image attachment: hover shows a live thumbnail (GIFs animate),
            // click opens the zoom/pan viewer — the copy-path action moves
            // into the viewer's toolbar. The hover card replaces the textual
            // protocol tip (both would stack on the same hover).
            chip.dataset.image = '1';
            const src = attachmentFileUrl(block.root, file.path);
            const card = document.createElement('div');
            card.className = 'dshca-hover-card';
            const thumb = document.createElement('img');
            thumb.src = src;
            thumb.alt = file.path;
            thumb.decoding = 'async';
            // The send directory may be gone (cleanup, another machine):
            // drop the card silently and fall back to the plain chip.
            thumb.addEventListener('error', () => { card.remove(); delete chip.dataset.image; });
            card.appendChild(thumb);
            const cardName = document.createElement('div');
            cardName.className = 'dshca-hover-name';
            cardName.textContent = `${file.path} · ${file.size} · 点击查看大图`;
            card.appendChild(cardName);
            chip.appendChild(card);
            chip.addEventListener('click', event => {
              event.stopPropagation();
              openImageViewer({ src, name: file.path, path: absolute });
            });
          } else {
    ...
    function apply(ctx) {
      ...
      // (multi-instance refactor); fall back to the legacy list snapshot.
      let uiSession;
      try { uiSession = ctx.get('uiSession'); } catch (cause) { uiSession = undefined; }

      // --- dsh-paste-input 自动版本检查 + 更新 chip（纯客户端，无宿主端点） ---
      ...
      // ── Version-check dedup / cooldown ─────────────────────────────
      // A 403 (GitHub rate-limit / unauth) or a network failure raises a console
      // error per fetch and re-fires on every reload and retry. Cache each
      // verdict in localStorage with a per-status TTL: an OK result lives 10
      // minutes, a transient network failure 60s, a hard 403 a full 5 minutes
      // (it will not clear on retry soon). Within the window the check returns
      // the cached verdict without touching the network, so a blocked check no
      // longer spams 403s.
      const LS_CHECK = 'dsh-paste-input:lastcheck';
      const TTL_OK = 10 * 60 * 1000;
      const TTL_NET = 60 * 1000;
      const TTL_BLOCKED = 5 * 60 * 1000;
      function readCheckCache() {
        try {
          const saved = localStorage.getItem(LS_CHECK);
          if (saved !== null) {
            const c = JSON.parse(saved);
            if (typeof c.at === 'number' && Number.isFinite(c.at)
              && (c.status === 'ok' || c.status === 'http' || c.status === 'net')
              && (typeof c.tag === 'string' || c.tag === undefined)) return c;
          }
        } catch (_) { /* storage unavailable */ }
        return null;
      }
      function writeCheckCache(value) {
        try { localStorage.setItem(LS_CHECK, JSON.stringify(value)); } catch (_) { /* storage unavailable */ }
        checkCache = value;
      }
      let checkCache = readCheckCache() || { at: 0, tag: undefined, status: 'net' };
      function cacheAge() { return Date.now() - checkCache.at; }
      function cacheTtl() { return checkCache.status === 'ok' ? TTL_OK : checkCache.status === 'http' ? TTL_BLOCKED : TTL_NET; }

      async function latestFromTags() {
        // jsdelivr's CDN API (data.jsdelivr.com) — no rate limit, CORS-enabled, unlike
        // api.github.com which 403s unauthenticated callers within ~60 req/hr.
        try {
          const res = await fetch('https://data.jsdelivr.com/v1/packages/gh/' + MIRROR, { signal: AbortSignal.timeout(8000) });
          if (res.ok) {
            const data = await res.json();
            const stable = (Array.isArray(data?.versions) ? data.versions : [])
              .map(v => String(v))
              .filter(n => /^v\d+\.\d+\.\d+$/.test(n));
            const tag = stable.length > 0 ? stable.reduce((a, b) => (semverCmp(b, a) > 0 ? b : a)) : undefined;
            return { tag, status: 'ok' };
          }
          return { tag: undefined, status: 'http' }; // !ok (e.g. 403)
        } catch (_) { return { tag: undefined, status: 'net' }; }
      }
      async function latestFromRaw() {
        try {
          const res2 = await fetch('https://raw.githubusercontent.com/' + MIRROR + '/main/package.json', { signal: AbortSignal.timeout(8000) });
          if (res2.ok) { const j = await res2.json(); const v = /^\d+\.\d+\.\d+$/.test(j.version) ? ('v' + j.version) : undefined; return { tag: v, status: 'ok' }; }
          return { tag: undefined, status: 'http' };
        } catch (_) { return { tag: undefined, status: 'net' }; }
      }
      /** Fetch the newest tag, honoring the cooldown cache (force skips it). */
      async function fetchLatestTag(force) {
        if (!force && checkCache.at !== 0 && cacheAge() < cacheTtl()) return checkCache.tag;
        // Parallel: total latency is the slowest source, not their sum.
        const [a, b] = await Promise.all([latestFromTags(), latestFromRaw()]);
        let status = 'net';
        if (a.status === 'http' || b.status === 'http') status = 'http';
        if (a.status === 'ok' || b.status === 'ok') status = 'ok';
        const tag = (a.status === 'ok' && a.tag) ? a.tag : (b.status === 'ok' && b.tag) ? b.tag : undefined;
        writeCheckCache({ at: Date.now(), tag, status: tag ? 'ok' : status });
        return tag;
      }
      function promptText(tag) {
        return '帮我更新 dsh-paste-input 插件到 ' + tag + '，步骤：'
          ...
          + '\n1. 执行 dsh plugin --profile web add \'' + PKG + '@github:' + MIRROR + '#' + tag + '\'（首次可能被 pnpm 11 拦截构建脚本，则先在 ~/.dsh/profiles/web 执行 pnpm approve-builds --all）'
          + '\n2. 完成后提醒我硬刷新浏览器（Ctrl/Cmd+Shift+R）'
          + '\n3. 遇到安装失败 / 版本不匹配 / 启动报错：先查 https://github.com/' + MIRROR + ' README 的「版本兼容」与「已知限制」章节再排查';
      }
      function relayoutUpdateChips() {
        const chips = document.querySelectorAll('[data-update-chip]');
        let next = 12;
        for (const ch of chips) { ch.style.bottom = next + 'px'; next += ch.getBoundingClientRect().height + 8; }
      }
      async function startUpdateChip() {
        const tag = await fetchLatestTag();
        if (!tag) { renderOfflineChip(); return; }
        if (semverCmp(tag, PLUGIN_VERSION) <= 0) {
          // The fetched tag can lag behind the running bundle right after a
          // push (GitHub tags/raw CDN cache): claim "latest" with whichever
          // of the two is newer, never an older fetched tag.
          const shown = semverCmp(tag, PLUGIN_VERSION) < 0 ? PLUGIN_VERSION : tag;
          renderCurrentChip('v' + String(shown).replace(/^v/, ''));
          return;
        }
        renderChip(tag);
      }
      function renderChip(tag) {
        if (document.querySelector('[data-update-chip="' + UPDATE_ID + '"]')) return;
        const el = document.createElement('div');
        el.setAttribute('data-update-chip', UPDATE_ID);
        el.setAttribute('role', 'button');
        el.style.cssText = 'position:fixed;left:12px;z-index:2147483000;display:inline-flex;align-items:center;gap:8px;padding:6px 10px;border:1px solid #4a7dff;border-radius:10px;background:#1e2430;color:#cfe0ff;font:12px/1.4 system-ui,Segoe UI,sans-serif;cursor:pointer;box-shadow:0 6px 18px rgba(0,0,0,.35);';
        const lbl = document.createElement('span');
        lbl.style.cssText = 'pointer-events:none;';
        lbl.textContent = '⟳ paste-input 新版本 ' + tag + ' 可用，点击更新';
        const cls = document.createElement('button');
        cls.textContent = '×';
        cls.setAttribute('aria-label', '关闭');
        cls.style.cssText = 'pointer-events:auto;border:0;background:transparent;color:#8fa3c8;font:inherit;cursor:pointer;padding:0 2px;line-height:1;';
        cls.addEventListener('click', function (ev) { ev.stopPropagation(); el.remove(); relayoutUpdateChips(); });
        el.appendChild(lbl); el.appendChild(cls);
        el.addEventListener('click', () => {
          const txt = promptText(tag);
          try { navigator.clipboard?.writeText(txt); } catch (_) { /* clipboard unavailable */ }
          lbl.textContent = '已复制更新提示词到剪贴板，粘贴发送即可';
      ...
      function renderOfflineChip() {
        if (document.querySelector('[data-update-chip="' + UPDATE_ID + '"]')) return;
        var blocked = checkCache.status === 'http';
        var el = document.createElement('div');
        el.setAttribute('data-update-chip', UPDATE_ID);
        el.setAttribute('title', blocked
          ? 'GitHub 拒绝本次版本检查（可能限流/未授权）；已缓存，避免反复请求'
          : '无法连接 GitHub 查询新版本（可能是网络不可达）');
        el.style.cssText = 'position:fixed;left:12px;z-index:2147483000;display:inline-flex;align-items:center;gap:8px;padding:6px 10px;border:1px solid #4a5060;border-radius:10px;background:#22252c;color:#9aa3b5;font:12px/1.4 system-ui,Segoe UI,sans-serif;cursor:pointer;box-shadow:0 4px 12px rgba(0,0,0,.3);';
        var lbl = document.createElement('span');
        lbl.style.cssText = 'pointer-events:none;';
        lbl.textContent = blocked
          ? '⚠ paste-input 版本检查暂不可用（GitHub 拒绝访问），已缓存'
          : '⚠ paste-input 版本检查失败（网络不可达），点击重试';
        var rty = document.createElement('button');
        rty.textContent = '重试';
        rty.setAttribute('aria-label', '重试版本检查');
        rty.style.cssText = 'pointer-events:auto;border:0;background:transparent;color:#8fa3c8;font:inherit;cursor:pointer;padding:0 2px;line-height:1;';
        var cls = document.createElement('button');
        cls.textContent = '×';
        cls.setAttribute('aria-label', '关闭');
        cls.style.cssText = 'pointer-events:auto;border:0;background:transparent;color:#8fa3c8;font:inherit;cursor:pointer;padding:0 2px;line-height:1;';
        cls.addEventListener('click', function (ev) { ev.stopPropagation(); el.remove(); relayoutUpdateChips(); });
        el.appendChild(lbl); el.appendChild(rty); el.appendChild(cls);
        var retrying = false;
        var retryOnce = function () {
          if (retrying) return;
          retrying = true;
          lbl.textContent = '版本检查中…';
          fetchLatestTag(true).then(function (t) {
            retrying = false;
            if (!t) { lbl.textContent = '⚠ paste-input 仍无法查询新版本'; return; }
            el.remove(); relayoutUpdateChips();
            if (semverCmp(t, PLUGIN_VERSION) > 0) renderChip(t);
            else renderCurrentChip('v' + String(semverCmp(t, PLUGIN_VERSION) < 0 ? PLUGIN_VERSION : t).replace(/^v/, ''));
          });
        };
        rty.addEventListener('click', function (ev) { ev.stopPropagation(); retryOnce(); });
      ...
      function renderCurrentChip(tag) {
        ...
        el.style.cssText = 'position:fixed;left:12px;z-index:2147483000;display:inline-flex;align-items:center;gap:8px;padding:6px 10px;border:1px solid #2f5d3a;border-radius:10px;background:#1c2a22;color:#9fd8ae;font:12px/1.4 system-ui,Segoe UI,sans-serif;cursor:pointer;box-shadow:0 4px 12px rgba(0,0,0,.3);';
        var lbl = document.createElement('span');
        lbl.style.cssText = 'pointer-events:none;';
        lbl.textContent = '\u2713 paste-input \u5df2\u662f\u6700\u65b0\u7248\u672c ' + tag;
        el.appendChild(lbl);
        el.addEventListener('click', function () { el.remove(); relayoutUpdateChips(); });
        document.body.appendChild(el);
        relayoutUpdateChips();
        setTimeout(function () { el.remove(); relayoutUpdateChips(); }, 4000);
      }
      void startUpdateChip();
      // --- /自动版本检查 ---
      ...
      const onPaste = event => {
        ...
        const proceed = () => {
          ...
          if (current === undefined) { showToast('请先打开一个会话。'); return; }
          const sessionId = current.sessionId;
          let items;
          try { items = renamedPastedItems(filesFromList(files)); } catch (cause) {
            showToast(cause instanceof Error ? cause.message : String(cause));
            return;
          }
          add(sessionId, items, current.scope).catch(cause => {
            showToast(cause instanceof Error ? cause.message : String(cause));
          });
        };
      ...
      const onDrop = event => {
        ...
        filesFromDrop(event.dataTransfer).then(
          items => add(sessionId, items, current.scope).catch(cause => {
            showToast(cause instanceof Error ? cause.message : String(cause));
          }),
      ...
      const inputForScope = scope => {
        if (scope === undefined) throw new Error('Attachment session scope is not active');
      ...
      // dsh 0.1.6-alpha.2: the current session comes from uiSession.current
      // ({ key: sessionId, ctx: session scope }); alpha.1 fell back to the
      // sessions.list 'current' field that the multi-instance refactor removed.
      const currentSession = () => {
        if (uiSession !== undefined) {
          const snap = uiSession.current.getSnapshot();
          if (snap !== undefined && snap.key !== undefined) return { sessionId: snap.key, scope: snap.ctx };
        }
        const sessionId = sessions.list?.getSnapshot()?.current;
        return sessionId === undefined ? undefined : { sessionId, scope: sessions.scope(sessionId) };
      };

      const add = async (sessionId, items, currentScope) => {
        validateItems(items);
        const input = currentScope !== undefined ? inputForScope(currentScope) : inputFor(sessionId);
        ...
        // Same-name files across pastes get a (2), (3)… suffix instead of
        // colliding: identical chip clipboardTexts break the composer's
        // reconciliation (the second paste errors and can displace the
        // first chip), and the user expects paste_image(2).png anyway.
        // Authoritative name set: the LIVE composer chips (survives a record
        // lost to the alive-subscription's momentary empty-occurrence flicks)
        // plus the session's records (covers an in-flight record whose chip
        // has not landed yet).
        const taken = new Set(
        ...
        for (const record of records.values()) {
          if (record.sessionId === sessionId) taken.add(record.label);
        }
        const uniqueLabel = label => {
          if (!taken.has(label)) { taken.add(label); return label; }
          const dot = label.lastIndexOf('.');
          const stem = dot > 0 ? label.slice(0, dot) : label;
          const extension = dot > 0 ? label.slice(dot) : '';
          let n = 2;
          while (taken.has(`${stem}(${n})${extension}`)) n++;
        ...
        for (const item of items) {
          const ref = id();
          const label = uniqueLabel(item.path);
          const record = { ref, sessionId, items: [{ ...item, path: label }], total: item.file.size, label, status: 'ready', uploaded: 0 };
          // Insertion point in DETECT coordinates: snapshot.draft is the
          // clipboard projection (a chip spans its whole clipboardText) but
          // the span guard works where a chip is one U+FFFC, so fold every
          // live chip's excess (length - 1) out of the clipboard length —
          // after the first attachment the raw draft length points past the
          // detect text and insertReference rejects the span.
          let detectEnd = snapshot.draft.length;
          for (const occurrence of snapshot.occurrences) detectEnd -= ((occurrence.length ?? 1) - 1);
          records.set(ref, record);
          attachPersist(record);
          persistSnapshot();
          const accepted = input.insertReference({
            source: SOURCE,
            ref,
            // rc.8 composer chips render "@" + the full label inline and take
            // an official appearance icon; the full path and size stay in our
            // dock chip above the composer.
            label: composerLabel(label),
            appearance: 'file',
      ...
      const remove = (sessionId, occurrence) => {
        ...
        // Occurrence offsets/lengths are CLIPBOARD-projection coordinates
        // (a chip spans its whole clipboardText), while consumeToken's span
        // guard works in DETECT coordinates (a chip is exactly one U+FFFC).
        // Passing clipboard offsets there silently failed and left the chip
        // behind; map to detect coordinates first: every earlier chip
        // contributes (length - 1) fewer characters in the detect projection.
        const ordered = snapshot.occurrences;
        const index = ordered.findIndex(item => item.occurrenceId === occurrence.occurrenceId);
        let removed = false;
        if (index >= 0 && typeof input.consumeToken === 'function') {
          let detectStart = ordered[index].offset;
          for (let j = 0; j < index; j++) detectStart -= ((ordered[j].length ?? 1) - 1);
          removed = input.consumeToken({
            kind: 'span',
            span: { start: detectStart, end: detectStart + 1, draftRev: snapshot.draftRev },
          });
        }
        if (!removed) {
          // Fallback: slice the chip's clipboard range out of the whole draft.
      ...
      // Whole-chip edit guard: an attachment's inline chip must be edited
      // (deleted or replaced) as one unit. The input machine has no
      // pre-edit hook — a partial edit (typing inside the label,
      // backspacing one character) reconciles the occurrence away and
      // demotes the chip to plain draft text. So partial edits are
      // intercepted on the composer textarea before the machine sees
      // them: the selection expands to the whole chip, and the user's
      // next keypress removes or replaces it entirely. A selection that
      // already covers the whole chip passes through.
      const onBeforeInput = event => {
        const target = event.target;
        if (!(target instanceof HTMLTextAreaElement)) return;
        const type = event.inputType ?? '';
        if (!type.startsWith('insert') && !type.startsWith('delete')) return;
        const current = currentSession();
        if (current === undefined) return;
        let input;
        try { input = inputFor(sessionId); } catch { return; }
        const snapshot = input.state.getSnapshot();
        if (snapshot.occurrences === undefined || snapshot.occurrences.length === 0) return;
        // Identify the composer textarea: its value mirrors the draft,
        // which unrelated textareas (queue row editors) never match.
        if (target.value !== snapshot.draft) return;
        const ss = target.selectionStart ?? 0;
        const se = target.selectionEnd ?? 0;
        // The draft range the edit would touch, before reconcile runs.
        let lo = ss;
        let hi = se;
        if (ss === se) {
          if (type.endsWith('Backward')) { lo = ss - 1; hi = ss; }
          else if (type.endsWith('Forward')) { lo = ss; hi = ss + 1; }
          else if (!type.startsWith('insert')) return;
        }
        for (const occurrence of snapshot.occurrences) {
          if (occurrence.source !== SOURCE) continue;
          const rs = occurrence.offset;
          const re = rs + (occurrence.length ?? 1);
          // A collapsed insertion only matters strictly inside the chip;
          // typing at either boundary is ordinary adjacent text entry.
          const overlaps = ss === se && type.startsWith('insert')
            ? lo > rs && lo < re
            : lo < re && hi > rs;
          if (!overlaps) continue;
          if (ss <= rs && se >= re) return; // whole-chip delete/replace: allowed
          event.preventDefault();
          // Expand the selection over the whole chip so the next keypress
          // deletes or replaces it as one unit.
          target.setSelectionRange(Math.min(ss, rs), Math.max(se, re));
          return;
        }
      };
      document.addEventListener('beforeinput', onBeforeInput, true);
      ctx.effect(() => () => document.removeEventListener('beforeinput', onBeforeInput, true), 'dsh-paste-input: chip edit guard');

      // 标题栏开关组件：切换粘贴记录的 sessionStorage 持久化（默认关）
      function PersistToggle() {
        const [on, setOn] = React.useState(persistRecords);
        return React.createElement('button', {
          title: '粘贴记录持久化到 sessionStorage（默认关；单文件 > 1 MiB 不持久化，配额上限约 3 MiB）',
          onClick: () => {
            const next = !on;
            setOn(next);
            setPersistFlag(next);
          },
          style: { fontSize: '11px', padding: '1px 6px', cursor: 'pointer', marginRight: '6px' },
        }, on ? '\uD83D\uDCCE 记录持久化 开' : '\uD83D\uDCCE 记录持久化 关');
      }
      ...
      ctx.inject(['slots', 'conversation', 'sessions', 'inputTriggers'], scope => {
        scope.slots.inject('conversation.input.left', () => scope.slots.register({
          ...
          order: -100,
          inject: sessionId => ({ add: items => { const current = currentSession(); add(current !== undefined ? current.sessionId : sessionId, items, current?.scope) } }),
        }, AttachButton));
        ...
        // 输入框左下（回形针旁）：粘贴记录是否持久化到 sessionStorage（默认关；大文件自动跳过）
        scope.slots.inject('conversation.input.left', () => scope.slots.register({
          name: 'conversation.input.left',
          id: 'dsh-paste-input-persist-toggle',
          order: -99,
        }, PersistToggle));
      });
    }
